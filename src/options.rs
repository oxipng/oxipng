...
use crate::{deflate::Deflater, filters::RowFilter, headers::StripChunks, interlace::Interlacing};
...
pub struct Options {
    ...
    pub filters: IndexSet<RowFilter>,
...
impl Options {
    ...
    fn apply_preset_5(mut self) -> Self {
        self.fast_evaluation = false;
        self.filters.insert(RowFilter::Up);
        self.filters.insert(RowFilter::MinSum);
        self.filters.insert(RowFilter::BigEnt);
        self.filters.insert(RowFilter::Brute);
        if let Deflater::Libdeflater { compression } = &mut self.deflater {
            *compression = 12;
        }
        self
    ...
    fn apply_preset_6(mut self) -> Self {
        self.filters.insert(RowFilter::Average);
        self.filters.insert(RowFilter::Paeth);
        self.apply_preset_5()
    }
}
...
impl Default for Options {
    fn default() -> Self {
        ...
        Self {
            fix_errors: false,
            force: false,
            filters: indexset! {RowFilter::None, RowFilter::Sub, RowFilter::Entropy, RowFilter::Bigrams},
